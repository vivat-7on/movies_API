...
from etl.es.model import EsGenre, EsPerson, FilmEsDocument
...
def transform_film_work(film_work: FilmWorkDTO) -> FilmEsDocument:
    directors = [
        person for person in film_work.persons if person.role == Roles.DIRECTOR
    ]
    actors = [person for person in film_work.persons if person.role == Roles.ACTOR]
    writers = [person for person in film_work.persons if person.role == Roles.WRITER]
